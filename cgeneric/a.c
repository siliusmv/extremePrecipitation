...
double * probit_mean_model(inla_cgeneric_cmd_tp cmd,
			   double * theta,
  ...
  if (theta) {
    ...
    assert(count == n_free);
    mu0 = exp(theta_full[0]);
    lambda= exp(theta_full[1]);
    kappa= exp(theta_full[2]);
  } else {
    mu0 = lambda = kappa = NAN;
  }
  ...
  case INLA_CGENERIC_MU:
    {
      ...
      ret[0] = n;
      for (int i = 0; i < n; ++i) {
	ret[1 + i] = mu0 * exp(-pow(dist[i] / lambda, kappa));
      }
      break;
    }
...
double * a_model3(inla_cgeneric_cmd_tp cmd,
		 double * theta,
		 inla_cgeneric_data_tp * data) {
  double * ret = NULL;
  ...
  // the parameters are fixed or estimated.
  // ==========================================================
  double lambda0, lambda_lambda, kappa_lambda, kappa0, delta_kappa;
  assert(n_theta == 5);
  if (theta) {
    double theta_full[5];
    int count = 0;
    ...
    kappa_lambda = exp(theta_full[2]);
    kappa0 = exp(theta_full[3]);
    delta_kappa = exp(theta_full[4]);
  } else {
    lambda0 = lambda_lambda = kappa_lambda = kappa0 = delta_kappa = NAN;
  }
  ...
  case INLA_CGENERIC_MU:
    {
      ...
      double lambda, kappa;
      for (int i = 0; i < n; i++) {
	lambda = lambda0 * exp(-pow(y0[i] / lambda_lambda, kappa_lambda));
	kappa = kappa0 - delta_kappa * y0[i];
  ...
  case INLA_CGENERIC_LOG_PRIOR:
	...
	++count;
      ...
      if (!is_fixed[2]) { // kappa_lambda
	prior = data->doubles[prior_start + count]->doubles;
	ret[0] += gaussian_const - log(prior[1]) -
	  pow(log(kappa_lambda) - prior[0], 2) / (2 * pow(prior[1], 2));
	++count;
      }
      if (!is_fixed[3]) { // kappa0
	prior = data->doubles[prior_start + count]->doubles;
	ret[0] += gaussian_const - log(prior[1]) -
	  pow(log(kappa0) - prior[0], 2) / (2 * pow(prior[1], 2));
	++count;
      }
      if (!is_fixed[4]) { // delta_kappa
	prior = data->doubles[prior_start + count]->doubles;
	ret[0] += gaussian_const - log(prior[1]) -
	  pow(log(delta_kappa) - prior[0], 2) / (2 * pow(prior[1], 2));
	++count;
      }
    }
  case INLA_CGENERIC_QUIT:
...
double * a_model2(inla_cgeneric_cmd_tp cmd,
		 double * theta,
		 inla_cgeneric_data_tp * data) {
  double * ret = NULL;
  ...
  // the parameters are fixed or estimated.
  // ==========================================================
  double lambda0, lambda1, kappa0, kappa1;
  assert(n_theta == 4);
  if (theta) {
    double theta_full[4];
    int count = 0;
    ...
    assert(count == n_free);
    lambda0 = exp(theta_full[0]);
    lambda1 = exp(theta_full[1]);
    kappa0 = exp(theta_full[2]);
    kappa1 = exp(theta_full[3]);
  } else {
    lambda0 = lambda1 = kappa0 = kappa1 = NAN;
  }
  ...
  case INLA_CGENERIC_MU:
    {
      ...
      double lambda, kappa;
      for (int i = 0; i < n; i++) {
	lambda = softplus(lambda0 - lambda1 * y0[i], 10);
	kappa = softplus(kappa0 - kappa1 * y0[i], 10);
	ret[1 + i] = y0[i] * exp(-pow(dist[i] / lambda, kappa));
      }
      break;
    }
...
double * const_model(inla_cgeneric_cmd_tp cmd,
		     double * theta,
		     inla_cgeneric_data_tp * data) {
  double * ret = NULL;
  ...
  // The fixed precision used in the precision matrix of a
  // double high_prec = exp(15);
  double high_prec = exp(5);

  // ==========================================================
  // Assert that all the input variables look as they shold
  // ==========================================================
  assert(!strcmp(data->doubles[0]->name, "prior"));
  double prior_mu = data->doubles[0]->doubles[0];
  double prior_sigma = data->doubles[0]->doubles[1];
  assert(!strcmp(data->doubles[1]->name, "init"));
  double init = data->doubles[1]->doubles[0];
  ...
  case INLA_CGENERIC_LOG_PRIOR:
    {
      ...
      // Only add terms for the parameters that are not fixed
      ret = Malloc(1, double);
      ret[0] = gaussian_const - log(prior_sigma) -
	pow(mean - prior_mu, 2) / (2 * prior_sigma * prior_sigma);
      break;
    }
  case INLA_CGENERIC_QUIT:
  default:
...
double * probit_mean_model(inla_cgeneric_cmd_tp cmd,
  ...
  assert(!strcasecmp(data->doubles[1]->name, "dist"));
  double * dist = data->doubles[1]->doubles;
  ...
  if (theta) {
    ...
    assert(count == n_free);
    scale = exp(theta_full[0]);
    lambda = exp(theta_full[1]);
    kappa = exp(theta_full[2]);
    tau = exp(theta_full[3]);
  } else {
    scale = lambda = kappa = tau = NAN;
  }
  ...
  case INLA_CGENERIC_Q:
    {
      ...
      // The precision matrix is just a diagonal matrix with each element equal to tau
      ret = Malloc(2 + n, double);
      ret[0] = -1;			       // code for optimized output
      ret[1] = n;			       // number of (i <= j)
      for (int i = 0; i < n; i++) {
  ...
  case INLA_CGENERIC_MU:
    {
      ...
      ret[0] = n;
      for (int i = 0; i < n; i++) {
	ret[1 + i] = scale * exp(-pow(dist[i] / lambda, kappa));
      }
      break;
    }
  ...
  case INLA_CGENERIC_LOG_PRIOR:
	...
	prior = data->doubles[prior_start + count]->doubles;
	ret[0] += gaussian_const - log(prior[1]) -
	  pow(log(scale) - prior[0], 2) / (2 * pow(prior[1], 2));
	++count;
      }
      if (!is_fixed[1]) { // lambda
	prior = data->doubles[prior_start + count]->doubles;
	...
	prior = data->doubles[prior_start + count]->doubles;
	ret[0] += gaussian_const - log(prior[1]) -
	  pow(log(tau) - prior[0], 2) / (2 * pow(prior[1], 2));
	++count;
      }
      assert(count == n_free);
      break;
...
double * probit_mean_model2(inla_cgeneric_cmd_tp cmd,
			    double * theta,
			    inla_cgeneric_data_tp * data) {
  double * ret = NULL;
  ...
  if (theta) {
    ...
    assert(count == n_free);
    lag = exp(theta_full[0]);
    deg = exp(theta_full[1]);
  } else {
    lag = deg = NAN;
  }
  ...
  case INLA_CGENERIC_LOG_PRIOR:
	...
	prior = data->doubles[prior_start + count]->doubles;
	ret[0] += gaussian_const - log(prior[1]) -
	  pow(log(lag) - prior[0], 2) / (2 * pow(prior[1], 2));
	++count;
      }
      if (!is_fixed[1]) { // deg
	prior = data->doubles[prior_start + count]->doubles;
	ret[0] += gaussian_const - log(prior[1]) -
	  pow(log(deg) - prior[0], 2) / (2 * pow(prior[1], 2));
	++count;
      }
      assert(count == n_free);
      break;
